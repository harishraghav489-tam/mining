...
// 1. Build Admin Dashboard with basePath: /mining/admin
console.log('📦 1/2 Building Admin Dashboard with basePath: /mining/admin ...');
execSync(`"${nextBin}" build frontend/admin`, {
  stdio: 'inherit',
  cwd: rootDir,
  env: { ...process.env, NEXT_PUBLIC_BASE_PATH: '/mining/admin' },
});
const adminOut = path.join(rootDir, 'frontend', 'admin', 'out');
fs.cpSync(adminOut, path.join(outDir, 'admin'), { recursive: true });

// 2. Build User Safety PWA with basePath: /mining/user
console.log('📱 2/2 Building Worker Safety PWA with basePath: /mining/user ...');
execSync(`"${nextBin}" build frontend/user`, {
  stdio: 'inherit',
  cwd: rootDir,
  env: { ...process.env, NEXT_PUBLIC_BASE_PATH: '/mining/user' },
});
...
console.log('✅ GitHub Pages multi-app bundle with base paths successfully created in out-pages/!');

...
export function GisMineMap({
  ...
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      ...
      insarPoints.forEach((pt) => {
        ...
        circle.bindTooltip(
          `<div class="text-[11px] font-mono">
            <strong>Sentinel-1 InSAR ${t('insar.subsidence')}</strong><br/>
            ${pt.zone} • ${t('insar.rate')}: <strong style="color:${color}">${pt.rate} ${t('insar.mmPerYear')}</strong>
          </div>`,
      ...
      evacPolyline.bindTooltip(
        `<strong>${t('map.evacuationRoute')} (${t('map.zoneLabel')} → ${t('map.assemblyPoint')})</strong>`,
        { sticky: true }
      );
      ...
      musterMarker.bindTooltip(`<strong>${t('map.assemblyPoint')}</strong>`, {
        permanent: true,
        direction: 'right',
      ...
      if (isUserView) {
        ...
        workerMarker.bindTooltip(`<strong>${t('map.workerLocation')} (Zone 01)</strong>`, {
          permanent: true,
          direction: 'top',
  ...
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      ...
      visibleNodes.forEach((node) => {
        ...
        const statusLabel = isNodeCrit ? t('risk.critical') : isNodeWarn ? t('risk.warning') : t('status.normal');
          ...
          <div class="p-2 font-sans min-w-[200px] sm:min-w-[230px] text-slate-900">
            ...
            <div class="space-y-1 text-xs">
              ...
              <div class="flex justify-between text-[9px] text-slate-500 pt-0.5">
                <span>${t('sensor.signal')}: <strong>${node.signalRssi} dBm</strong></span>
                <span>${t('sensor.battery')}: <strong>${node.batteryLevel}%</strong></span>
              </div>
            </div>
          </div>
        `;
  ...
  }, [state.nodes, state.mode, activeZone, isLeafletReady, state.language]);
    ...
    >
      ...
      <div className="absolute bottom-2 left-2 z-20 bg-slate-950/90 backdrop-blur-md border border-slate-800 rounded-lg p-2 text-white shadow-xl pointer-events-auto max-w-[220px] sm:max-w-xs text-[9px] sm:text-[10px]">
        ...
        <div className="flex justify-between text-[8px] font-mono text-slate-400">
          <span className="text-rose-400 font-bold">&gt;12mm ({t('alerts.alert')})</span>
          <span className="text-emerald-400 font-bold">&lt;3mm ({t('risk.safe')})</span>
        </div>
      </div>
    </div>
  );
